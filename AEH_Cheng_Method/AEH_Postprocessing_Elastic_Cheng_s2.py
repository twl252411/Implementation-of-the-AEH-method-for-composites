...
#
odb = openOdb(f'{new_job_name}_2.odb')
...
for iStep in range(6):
    file_name = f'{new_job_name}_sec_nd_disp_{iStep+1}.txt'
    F1V = (odb.steps[f"Step-{iStep + 1}"].frames[-1].fieldOutputs['U'].getTransformedField(datumCsys=dtm).
